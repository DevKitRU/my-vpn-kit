...
# Проверка других VPN-клиентов
CONFLICTING=""
for proc in Shadowrocket xray v2ray Clash Surge Hiddify; do
    if pgrep -x "$proc" > /dev/null 2>&1; then
        CONFLICTING="$CONFLICTING $proc"
    fi
done
# Старый sing-box без LaunchAgent
if pgrep -x sing-box > /dev/null 2>&1 && ! launchctl list | grep -q com.singbox.vpn; then
    CONFLICTING="$CONFLICTING sing-box(вне LaunchAgent)"
fi

if [[ -n "$CONFLICTING" ]]; then
    warn "Обнаружены работающие VPN-клиенты:$CONFLICTING"
    warn "Возможны конфликты портов и системного прокси."
    warn "Закрой их перед установкой (или оставь один активным пока ставим новый)."
    read -r -p "Продолжить? [y/N]: " ans
    [[ "$ans" != "y" ]] && { info "Остановлено"; exit 0; }
fi
...
# Очистка от whitespace и CR (частая причина ошибок при копипасте)
VLESS="${VLESS//$'\r'/}"
VLESS="${VLESS//$'\n'/}"
VLESS="${VLESS//$'\t'/}"
VLESS="$(echo "$VLESS" | xargs)"  # trim leading/trailing spaces
...
echo "╚══════════════════════════════════════════╝"
echo ""
echo -e "${YELLOW}!!! ВАЖНО для Claude Code:${NC}"
echo -e "${YELLOW}    Закрой VSCode ПОЛНОСТЬЮ (Cmd+Q — не просто закрытие окна!) и открой заново.${NC}"
echo -e "${YELLOW}    Переменные среды читаются процессом только при старте —${NC}"
echo -e "${YELLOW}    текущий Claude Code HTTPS_PROXY не увидит.${NC}"
echo ""
echo "Что дальше:"
echo "  1. VSCode: Cmd+Q → заново (см. выше, жёлтый текст)"
echo "  2. Если другие VPN (Hiddify и т.п.) больше не нужны — останови их"
...
echo "Troubleshooting: https://github.com/DevKitRU/my-vpn-kit/blob/main/docs/troubleshooting.md"
echo ""
